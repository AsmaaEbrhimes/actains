...
let idclothes = localStorage.getItem('idjalabia')
...
let array_product=[]
const savedproductcart = JSON.parse(localStorage.getItem('productjalabia'));
if (savedproductcart) {
...
function AddtoCartdress(id) {
    ...
    if (username && email) {
        ...
        if (!array_product.some((item) => item.id === id)) {
            ...
            localStorage.setItem('productjalabia', JSON.stringify(array_product));
            showCartShopping()
            ShowProductCart()
        }
    } else {
 ...
 const ShowProductCart=()=>{
    ...
    // let data_cart=JSON.parse(localStorage.getItem('product-jalabia'))
    let data_cart=JSON.parse(localStorage.getItem('productjalabia'))
...
let array = []
const savedData = JSON.parse(localStorage.getItem('favouritejalabia'));
if (savedData) {
    array = savedData
}
...
function Addtowishlist(id) {
    ...
    if (username && email) {
        ...
        if (!array.some((item) => item.id === id)) {
            ...
            localStorage.setItem('favouritejalabia', JSON.stringify(array));
            showProductWishlist()
        }
    } else {
        return false;
...
const showProductWishlist=()=>{
    ...
    let data_favourite= JSON.parse(localStorage.getItem('favouritejalabia'));
...
const increaseNumber = () => {
    ...
    let cartProducts = JSON.parse(localStorage.getItem('productjalabia'));
    ...
    localStorage.setItem('productjalabia', JSON.stringify(cartProducts));
    showCartShopping();
}
return false
...
const decreseNumber = () => {
    ...
    if(value > 0){
    inputNum.value = value;
    let cartProduct = JSON.parse(localStorage.getItem('productjalabia'));
    let productIndex = cartProduct.findIndex((item) => item.id === fiendProduct.id);
    if (productIndex !== -1) {
        cartProduct[productIndex].qty = value;
    }
    localStorage.setItem('productjalabia', JSON.stringify(cartProduct));
    showCartShopping();
}
return false
}
